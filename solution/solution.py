...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if scores_batch and all(
            entry.get("scores") for entry in scores_batch
        ):
            first_values = [
                next(iter(entry["scores"].values())) for entry in scores_batch
            ]
            positional_bias = all(v >= 0.8 for v in first_values) and len(
                first_values
            ) > 1
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            suggestion = suggestions[i] if i < len(suggestions) else "Review failure manually"
            rows.append(
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_map = {
            ...
            "irrelevant": "Add intent classification / prompt clarification to keep answers on-topic",
            "incomplete": "Increase chunk size or top-k retrieval to reduce context fragmentation",
            "off_topic": "Improve routing/intent detection to match answers to the right topic",
            "refusal": "Relax overly strict guardrails for in-scope questions",
            "unknown": "Add more granular failure_type classification for better triage",
        }

        suggestions = [
            suggestion_map[failure_type]
            for failure_type in categories
            if failure_type in suggestion_map
        ]

        fallback_suggestions = [
            "Add few-shot examples showing complete, well-grounded answers",
            "Expand the golden dataset with more edge cases similar to these failures",
            "Review the full pipeline end-to-end for systemic issues",
        ]
